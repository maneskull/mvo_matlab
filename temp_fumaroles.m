function DOUT=temp_fumaroles(mat,tlim,OPT,nograph,dirspec)
%CGPS	Plots all graphs from continuous GPS network.
%       CGPS without any option loads recent data from disk and updates all graphs on WEBOBS.
%
...
%
%   Authors: PJS, MVO
%   after: F. Beauducel / IPGP + H. Odbert / MVO
%   Created: 2018-03-13
%   Updated: 2018-03-13
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Adding routine to read fumarole temps data from one csv file.
% Need only read the csv file once.
datafile = sprintf('%s.csv', 'FLIR_Derived_Fumarole_Temps');
myfile = [G.don,'/',datafile];
...
%     
	d = [];
	for i=2:num_col
		if strcmp(sname, strtrim(dd.textdata{1,i}))
			d = dd.data(:, i-1);
			break;
		end
	end
...
%    	if mat < 1 % load from text file
    	...
    	%datafile = 'FLIR_Derived_Temps.csv  ';
    	%file = [pftp,'/',datafile];
